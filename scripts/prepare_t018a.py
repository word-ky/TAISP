...
def prepare(project, output):
    ...
    key=lambda i:(hashlib.sha256(f'20260918:{i}'.encode()).hexdigest(),i)
    ids=sorted(eligible,key=key)[:100]
    assert len(ids)==100 and not set(ids)&excluded
    records=[{'image_id':i,'path':str(images/infos[i]['file_name']),'file_name':infos[i]['file_name'],
              ...
              'block':j//25,'selection_hash':key(i)[0]} for j,i in enumerate(ids)]
    result={'split':'train2017','seed':20260918,'image_ids':ids,'images':records,
            'replication_blocks':{f'block{i}':ids[25*i:25*i+25] for i in range(4)},
            ...
            'provenance_sha256':{str(p.relative_to(project)):sha256(p) for p in (hpath,apath,ledgerpath)},
            'selection':"first100 sorted by (sha256('20260918:'+decimalID),ID) after frozen exclusions",
