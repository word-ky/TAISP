...
'''
    ...
    if not gate['passed']:
        report+='The T018-A developmental gain did not replicate. Under the frozen R030 decision rule, this exact fixed native pseudo-target formulation is not confirmed and should be closed pending research review, without tuning or cross-detector promotion.\n\n'
    report+=f"Clean native-minus-no-adapt AP is {a['clean_AP']['nativePT_ours']-a['clean_AP']['no_adapt']:+.9f}; the clean comparison against current Ours must be read alongside this raw baseline.\n\n"
