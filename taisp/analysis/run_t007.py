"""T007: source-only ISP adaptation, paired source/independent-target analysis."""
import argparse
import json
...
from .run_t006 import cosine
from taisp.tta.trust_radius import adapt_clip_radius


def diagnostics(gradient, oracle, before, after, lr):
    return {'g_det': oracle.tolist(), 'g_det_norm': oracle.norm().item(),
            'gradient_cosine': cosine(gradient, oracle),
            'coordinate_dot': (gradient*oracle).tolist(),
            'linear_prediction': (-lr*torch.dot(gradient, oracle)).item(),
...
def run(config, data_root, output, limit=None):
    ...
    meta.update(evaluated_image_ids=ids, smoke_limit=limit, target=target_metadata(target),
                ...
                loss='T005 fixed-ROI CE; hybrid direction from source, magnitude from current-phi CLIP, eps1e-12',
                ...
                trust_radius='every step gd*norm(gc)/(norm(gd)+eps); zero gd stays zero; no target/oracle',
                subset_sha256=__import__('hashlib').sha256((data.root/'subset.json').read_bytes()).hexdigest(),
        ...
        for index, image_id in enumerate(ids):
            ...
            for family, severity in cases:
                ...
                for variant in config['variants']:
                    ...
                    hybrid = variant == 'det_pseudo_clip_radius'
                    ...
                    result = (adapt_clip_radius(x, isp, guidance, generic, steps=cfg.steps, lr=cfg.lr, eps=config['radius_eps'])
                              if hybrid else adapt(x, isp, guidance, config=cfg))
                    torch.cuda.synchronize()
                    ...
                    with torch.no_grad():
                        images = {'1': isp(x, phi1), '3': result.enhanced}
                    ...
                    oracle_diagnostics = {d: diagnostics(gradient, gradients[d], before[d], after[d], cfg.lr) for d in detectors}
                    row = {'image_id': image_id, 'family': family, 'severity': severity, 'variant': variant,
                           ...
                           'source': oracle_diagnostics['source'], 'target': oracle_diagnostics['target'],
                           ...
                           'radius_eps': config['radius_eps'] if hybrid else None}
...
def main():
    ...
    parser.add_argument('--config', type=Path, default=Path('configs/t007.yaml'))
