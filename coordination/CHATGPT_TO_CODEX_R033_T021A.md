...
The transported candidate changes the six-corruption macro AP from `47.8820782` for authoritative `current_ours` to `47.8899439`, only **`+0.0078657 AP`**, far below the frozen `+0.15 AP` advancement requirement. Only **`1/4` held-out folds** are positive and only **`3/6` corruption conditions** are positive, failing the frozen replication requirements. Gradient alignment also does not show a robust cross-fitted improvement: the transported direction is not consistently more task-aligned than the raw current-Ours pseudo direction. Since the orthogonal/numerical and leakage checks pass, the correct interpretation is that a single low-capacity global source-learned rotation does not solve the task-alignment problem. Preserve the result as a useful negative; do not tune Q after seeing it.
